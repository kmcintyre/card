define(["table", "shoe", "hand_blackjack_player", "hand_blackjack_dealer", "deck"], function(table, shoe, hand_blackjack_player, hand_blackjack_dealer, deck) {
    ...
    table_blackjack.prototype.even = function(seat) {
    	if ( this.seats[seat].hand.bj() && this.dealer().options('insurance') ) {
    		this.seats[seat].bet = this.seats[seat].hand.bet * 2;
    		this.seats[seat].hand = null;
    	}
    }
    	...
    	if ( this.hands() ) {
    		...
    		for (var y = 1; y <= 2; y++) {
    			for (var x = 0; x < this.seats.length; x++) {
    				...
    				if ( this.seats[bump].hand && this.seats[bump].hand.cards.length < y ) {
    					console.log('card ' + y + ' to ' + bump + ' ' + this.seats[bump].hand);
    					this.seats[bump].hand.card(this.shoe.next());
    					return x;
    				}
    			}
    		}
