...
stdin.on('data', function(message) {
	...
	} else if ( t.players.length == 0 && t.tables.length == 0 && t.winners == 0 ) {
		console.log("start!");
		wss.start();		
...
wss.start = function() {
	...
	wss.broadcast( JSON.stringify({ "type": "msg" , "txt" : "tourney starting players: " + this.clients.length, "from": "server"}) );
...
wss.expectedplayers = function() {
	var ep = this.clients.length + (addconsole ? 1 : 0);
	console.info('expected players:' +  ep);
	wss.broadcast( JSON.stringify( {"type":"msg", "txt" : "expected players"} ) );	
};
